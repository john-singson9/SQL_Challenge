...
SELECT last_name AS 'Last Name', COUNT(actor_id) AS 'Actor Count'
FROM actor
GROUP BY last_name;
...
SELECT staff.staff_id, staff.last_name, staff.first_name, SUM(payment.amount) AS 'August 2005 Amount'
FROM staff
JOIN payment
...
SELECT film_actor.film_id, film.title, COUNT(film_actor.actor_id) AS 'Actor Number'
FROM film
...
SELECT film.film_id, film.title, COUNT(inventory.inventory_id) AS 'Inventory number'
FROM film
JOIN inventory
